...
class QuantizeEnv:
    def __init__(self, model, pretrained_model, compress_ratio, args, n_data_worker=16,
        ...
        # restore weight
        self.reset()
        print('=> original acc: {:.3f}'.format(self.org_loss))
    ...
    def reward(self, loss, w_size_ratio=None):
        ...
        #    return (loss - self.org_loss + 1. / w_size_ratio) * 0.1
    ...
    def _build_state_embedding(self):
        ...
        for i, ind in enumerate(self.quantizable_idx):
            m = module_list[ind]
            this_state = []
            if type(m) == nn.LayerNorm or type(m) == nn.LayerNorm:
                this_state.append([1.])  # layer type, 1 for conv_dw
